  ...
  return (
    <div className={`relative flex flex-col md:flex-row items-center justify-center w-full mb-12 md:mb-20 last:mb-0 ${isLeft ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
      ...
      <motion.div
        ...
        className={`w-full md:w-[48%] flex flex-col justify-center p-8 md:p-12 bespoke-card group ${isLeft ? 'md:text-left' : 'md:text-right'}`}
      >
        <div className={`flex flex-col mb-8 ${isLeft ? '' : 'md:items-end'}`}>
          <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-zinc-500 mb-2 group-hover:text-vibrant-cyan transition-colors duration-500 font-bold">{exp.duration}</span>
          <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tightest leading-[1] mb-2 text-white">{exp.role}</h3>
          <span className="text-[12px] md:text-[13px] font-bold uppercase tracking-[0.15em] text-zinc-500 italic">{exp.company}</span>
        </div>
        ...
        <ul className={`space-y-4 md:space-y-6 mb-12 ${isLeft ? 'text-left' : 'md:text-right'}`}>
          {exp.bullets.slice(0, 3).map((bullet, i) => (
            <li key={i} className={`flex gap-4 md:gap-6 group/bullet ${isLeft ? 'text-left' : 'md:flex-row-reverse md:text-right'}`}>
              <span className="text-white font-mono text-[11px] opacity-10 group-hover/bullet:opacity-100 transition-opacity">
                // 0{i + 1}
              </span>
              <p className="text-sm md:text-base text-zinc-400 leading-relaxed group-hover/bullet:text-white transition-colors">
                {bullet}
              </p>
            </li>
          ))}
        ...
        <div className={`flex flex-wrap gap-x-6 gap-y-3 pt-10 border-t border-white/10 ${isLeft ? '' : 'md:justify-end'}`}>
          {exp.tech.map((t) => (
            <span key={t} className={`text-[10px] md:text-[11px] font-mono tracking-[0.15em] uppercase font-black transition-all duration-500 hover:scale-110 cursor-default ${techColors[t] || 'text-zinc-500'}`}>
              {t}
            </span>
          ))}
        </div>
